...
app.post('/hmac/register', (req, res) => {
  ...
  const password = req.body.password;
  const username = req.body.username;
  const accesskey = auth.keyGen(160); // access key
  const sharedSecret = auth.keyGen(320); // sharedSecret
  const insert = "INSERT INTO users (email, password, username, accesskey, sharedsecret) VALUES \
    (:email, crypt(:password, gen_salt('bf', 8)), :username, :accesskey , :sharedSecret);";
  models.sequelize.query(insert, { replacements: { email, password, username, accesskey, sharedSecret } }).then(() => {
